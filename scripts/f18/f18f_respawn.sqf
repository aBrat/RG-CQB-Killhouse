...
if(isServer) then{
	while{_n == 1} do{
		if(!alive _vehicle) then { //true if vehicle is not alive
			...
			[[[_vehicle,_respawntime],"scripts\f18\f18f_init.sqf"],"BIS_fnc_execVM",false,false] spawn BIS_fnc_MP; //replacement for the old setVehicleInit, this does the same and causes the new vehicle to have the respawn script when created
